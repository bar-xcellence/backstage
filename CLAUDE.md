...
Owner/super_admin attach documents to an event — quote PDFs, LC invoice PDFs, floor plan JPEGs, menus, artwork. **The partner never sees any of it**: quotes and LC invoices are financial. Spec: `docs/superpowers/specs/2026-07-16-event-files-design.md`.

- `event_files` child table (cascade off `events`). Because it's a separate table, it is **not** classified in `partner-event-projection.ts` (that pins columns on `events` only) — isolation is enforced in the actions/routes instead. Category enum: `quote | lc_invoice | floor_plan | menu | artwork | other`.
- **Storage: a PRIVATE Vercel Blob store** (`@vercel/blob` ≥ 2.3; needs `BLOB_READ_WRITE_TOKEN`). Recipe images stay on UploadThing — they must stay publicly fetchable to render in brief emails/PDFs.
- **The store being private is the ONLY thing making these files private.** `access` cannot be pinned in the server-minted upload token (it's absent from the token constraint type; the browser sends it as an `x-vercel-blob-access` header and the blob hostname derives from it). So the `access: "private"` literal in each client `upload()` call is load-bearing — both call sites carry a comment saying so. A public store, or that literal flipped to `"public"`, puts client quotes on an unauthenticated URL.
- Upload: browser → Blob directly via `upload()`, tokened by `/api/event-files/upload` (role check in `onBeforeGenerateToken`, pins `allowedContentTypes` + `maximumSizeInBytes`). Direct upload bypasses the 4.5MB serverless body limit. Note `onUploadCompleted` is deliberately **not** defined — merely defining it makes the SDK embed a `callbackUrl` and webhook every upload, which can't reach localhost in dev.
- Read: **only** via `/api/event-files/[id]` — private blobs have no fetchable URL. The route checks the session next to the `get()` call (per Vercel's guidance never to rely on middleware here) and streams with `nosniff` + `Cache-Control: private, no-store`. Filenames go through an RFC 6266 `Content-Disposition`: Node's `Headers` throws on any char > U+00FF, so a plain `Aurora – floor plan.pdf` would otherwise 500 the route.
- Actions in `src/actions/event-files.ts` — all `requireRole("owner", "super_admin")`, including the read. (Contrast `getEventEquipment`, which allows the partner — do not copy that here.) `getEventFiles` deliberately narrows its `select()` so `blobUrl` never reaches the client payload.
- Validation: pure `validateEventFileInput()` in `src/lib/event-file-validation.ts` (TDD). `formatFileSize()` lives in `src/lib/file-size-format.ts` so client and form both import it without dragging a `"use client"` module graph.
- UI: Files tab on event detail inside the `!isPartner` block, grouped by category, house confirm-modal for delete. `quote-upload-field.tsx` gives the new-enquiry form a Quote PDF slot (`showQuoteUpload` prop — create only; the edit tab passes nothing and `updateEvent` reads no quote fields). `createEvent` inserts the row after the event, non-fatally: a failure logs and still redirects, since the event is the valuable write and the quote is re-uploadable from the Files tab.
- `deleteEvent` `del()`s the event's blobs **after** the `canDeleteEvent` guard and before the row cascade — before the guard would destroy a `completed` event's files; after the cascade would lose the URLs. `deleteEventFile` likewise removes the blob. Both swallow-and-log: `del()` is idempotent on a missing blob, so the catch only fires on network/auth errors, and a stuck row is worse than an orphan nothing can reach.
- Known gap: a quote uploaded on the new-enquiry form is orphaned if the form is abandoned (no row to walk from). Accepted — matches existing recipe-image behaviour; orphans are unreachable in a private store.
...
| 2026-06-19 | Cocktail garnishes missing from the spec (showed in Stock List only) — reported by Murdo for "Spiced Passionstar" (passionfruit quarter, pineapple leaf, edible gold dust) | The brief renders on four surfaces that must stay in sync, but the per-cocktail **spec** loop omitted a garnish block in 2 of 4: `brief-preview.tsx` (in-app) and `brief-pdf.tsx` (visual PDF) rendered ingredients/ice/straw but never `ec.garnishes`. The email template and text-only PDF already rendered garnishes inline. Data was always present (`getEventCocktails` loads garnishes; they were aggregated into the Stock List) — purely a missing-JSX render gap. | Added the garnish block to both spec loops (preview: "Garnish: …" line after ingredients; visual PDF: "(garnish)" lines, matching text-only PDF). All four surfaces now in sync. No partner leak — garnishes were already partner-visible via Stock List; no `partner-event-projection.ts` change. Regression test added in `brief-email-template.test.ts`. |
